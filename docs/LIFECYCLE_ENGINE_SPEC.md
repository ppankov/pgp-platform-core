...
1. **Generic by design.** The engine never references `Application`, `Plugin`, `Connector`,
   ...
   on `LifecycleDefinition`.
2. **`targetType` is an open string, never an enum.** Adding a new resource type
   (e.g. `document`, `template`, `ai_provider`) requires **no schema change**.
...
- `resourceType` (required), `resourceId` (required), `lifecycleDefinitionId` (required),
  `currentLifecycleStateId` (required). Unique by `(resourceType, resourceId)`.

### LifecycleApprovalRequest (approval service)
- `bindingId`, `lifecycleDefinitionId`, `transitionId`, `fromState`, `toState`,
...
- `bindingId`, `resourceType`, `resourceId`, `lifecycleDefinitionId`, `transitionId`,
  `fromState`, `toState`, `outcome`, `actorId`, `metadata`

## Permissions (frontend visibility; backend-enforced in Phase 2 backend authorization)

| Role | Definition management | Execution |
|---|---|---|
| Developer | read | read (runtime calls) |
| Solution Architect | read | read |
...
The runtime additionally enforces `transition.allowedRoles` at execution time.
...
- **`attachLifecycle`** `{ resourceType, resourceId, lifecycleDefinitionId }` → creates a
...
the runtime own `currentLifecycleStateId` in `LifecycleBinding`. The resource's own storage is
