...
// Plugin Engine — disablePlugin (Phase 5)
// Disables an installation. Idempotent when already disabled. Preserves configuration
// and history. Disabling an uninstalled installation is a no-op. Admin / super_admin only.
//
...
Deno.serve(async (req) => {
  try {
    ...
    if (role !== 'admin' && role !== 'super_admin') {
      return Response.json({ error: 'Not permitted to disable plugins' }, { status: 403 });
    }
    ...
    if (installation.uninstalledAt || installation.enabled === false) {
      return Response.json({ status: 'already_disabled', installation });
    }
    ...
    await publish(base44, 'plugin.disabled', installationId, {
      installationId,
      pluginId: installation.pluginId,
      organizationId: installation.organizationId,
    });

    return Response.json({ status: 'disabled', installation: updated });
  } catch (error) {
