...
// Connector Engine — disconnectConnectorConnection (Phase 6)
// Marks a connection disconnected. admin/super_admin only.
// Non-destructive: preserves the historical record and does not delete or resolve credential material.
...
Deno.serve(async (req) => {
  try {
    ...
    if (role !== 'admin' && role !== 'super_admin') {
      return Response.json({ error: 'Not permitted to disconnect connector connections' }, { status: 403 });
    }
    ...
    if (connection.status === 'disconnected') {
      return Response.json({ status: 'already_disconnected', connection });
    }

    const now = new Date().toISOString();
    const updated = await svc.entities.ConnectorConnection.update(connectionId, {
      status: 'disconnected', enabled: false, disconnectedAt: now,
    });

    await publish(base44, 'connector.connection_disconnected', connectionId, {
      connectionId: connectionId, connectorProviderId: connection.connectorProviderId,
      organizationId: connection.organizationId, status: 'disconnected', enabled: false,
    });

    return Response.json({ status: 'disconnected', connection: updated });
  } catch (error) {
