...
export default function DeadLetterQueuePage() {
  ...
  const load = async () => {
    setErr("");
    try {
      const res = await callFn("listBackgroundJobs", { status: ["dead_letter"], sort: "-deadLetteredAt", limit: 100 });
      setJobs(res.jobs || []);
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <ModuleHeader title={t("jobs.dead_letter")} description={t("jobs.dead_letter_desc")} />
      <Card>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t("jobs.field.lastError")}</TableHead>
                  <TableHead>{t("jobs.field.deadLetteredAt")}</TableHead>
              ...
              <TableBody>
                {jobs.map((j) => (
                  <TableRow key={j.id}>
                    ...
                    <TableCell className="text-xs text-destructive max-w-xs truncate">{j.lastError || "—"}</TableCell>
                    <TableCell className="text-xs text-muted-foreground">{fmt(j.deadLetteredAt)}</TableCell>
